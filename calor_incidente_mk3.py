...
for j in range(0, len(Ni), 1):
      for i in range(0, len(Posicao_orientacao), 1):
            ...
            ra = Posicao_orientacao.iloc[i, 0]
            inc = Posicao_orientacao.iloc[i, 1]
            ome = Posicao_orientacao.iloc[i, 2]
...
for i in range(0, len(prop_orb), 1):
    R = np.array([np.array(Posicao_orientacao.iloc[i][3]), np.array(Posicao_orientacao.iloc[i][4]), np.array(Posicao_orientacao.iloc[i][5])])
    R1 = R[0]
    R2 = R[1]
    R3 = R[2]
